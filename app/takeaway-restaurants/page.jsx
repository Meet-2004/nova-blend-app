...
import { setRestaurant, setMode, endSession } from "@/store/slices/dineInSlice";
...
export default function TakeawayRestaurants() {
  ...
  useEffect(() => {
    if (!phone) router.replace("/takeaway-login");
  }, [phone, router]);
  ...
  const pick = (id, name) => {
    if (cartCount > 0 && currentRestaurantId && currentRestaurantId !== id) {
      dispatch(showCartConflictModal({ id, name }));
      return;
    }
    confirmPick(id, name);
  };
        ...
        {cartConflictModal && (
          ...
          >
            <motion.div
              ...
              onClick={() => dispatch(hideCartConflictModal())}
            />
            <motion.div
              ...
              className="relative w-full max-w-sm glass-strong rounded-3xl p-6 ring-glow"
            >
              <div className="flex items-center gap-3 mb-4">
                <div className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-warning/15 text-warning">
                  <AlertTriangle className="h-5 w-5" />
                </div>
                <h3 className="font-semibold tracking-tight">Replace your cart?</h3>
              </div>
              <p className="text-sm text-muted-foreground leading-relaxed">
                You already have items from <strong className="text-foreground">another restaurant</strong>.
                Starting a new order will clear your existing cart.
              </p>
              <div className="mt-6 flex flex-col gap-2">
                <button
                  onClick={handleClearAndContinue}
                  className="w-full rounded-2xl bg-primary text-primary-foreground h-12 font-semibold text-sm ring-glow"
                >
                  Clear cart &amp; continue
                </button>
                <button
                  onClick={() => dispatch(hideCartConflictModal())}
                  className="w-full rounded-2xl border border-border h-12 text-sm font-medium text-muted-foreground hover:text-foreground transition"
                >
                  Keep existing cart
                </button>
              </div>
